...
public class RatingService {
    ...
    public boolean newsRating(NewsRatingRequest request) {
        Rating rating = new Rating(request.getUserId(), request.getNewsId(), request.getScore());
        updateRedis(rating);
    ...
    private void updateRedis(Rating rating) {
        if (jedis.exists("userId:" + rating.getUserId()) && jedis.llen("userId:" + rating.getUserId()) >= Constant.REDIS_NEWS_RATING_QUEUE_SIZE) {
            jedis.rpop("userId:" + rating.getUserId());
        }
        jedis.lpush("userId:" + rating.getUserId(), rating.getNewsId() + ":" + rating.getScore());
    }
