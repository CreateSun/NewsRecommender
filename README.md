...
## 题记
本项目是在大神tqz的电商推荐系统[点击查看原项目](https://github.com/ittqqzz/ECommerceRecommendSystem)的基础上进行二次修改，
1. 前端部分对页面的显示与交互逻辑进行了修改。服务端部分修改了部分Model层内容，并新增了部分接口以满足开发需要;
2. 推荐系统中修改了dataLoader部分的数据初始化的逻辑，修改了StatisticRecmmender的统计模块的计算逻辑
3. 新增了评分计算模块CalRating用于提取用户的行为日志并量化为用户对于新闻的评分，其他的部分基本保持了原作者的内容。
